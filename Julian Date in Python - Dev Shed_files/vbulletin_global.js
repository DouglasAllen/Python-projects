...
{
	window.open("/reputation.php?" + SESSIONURL + "p=" + postid, "Reputation", "toolbar=no, scrollbars=no, resizable=yes, width=400, height=245");
	return false;
}
...
function validatemessage(messageText, subjectText, minLength, maxLength, ishtml, tForm)
{
	...
	else if ((messageText.toLowerCase().match(/(<script)|(<\?)|(<\%)|(<style)/)) && (!messageText.toLowerCase().match(/(\[code)|(\[php)|(\[highlight=)|(\[hl=)/)))
	{
